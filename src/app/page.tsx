...
export default function Home() {
  ...
  // 靜態文字取代打字機效果
  const displayText = 'Full Stack Developer | Hong Kong';
  ...
  useEffect(() => {
    ...
    let workYears = currentDate.getFullYear() - startDate.getFullYear();
    let workMonths = currentDate.getMonth() - startDate.getMonth();
    ...
    setWorkExperience({ years: workYears, months: workMonths });

    // Calculate internship experience (4 months)
          ...
          <div className="max-w-3xl mx-auto px-4 font-mono">
            <div className="mb-12 cyber-card p-6 relative overflow-hidden group">
              <p className="relative z-10 text-base sm:text-lg text-yellow-300 leading-relaxed">
                ...
                <span className="font-bold text-2xl text-yellow-300 animate-pulse">
                  {workExperience.years}
                </span>{' '}
                {workExperience.years === 1 ? 'year' : 'years'} and{' '}
                <span className="font-bold text-2xl text-cyan-400 animate-pulse">
                  {workExperience.months}
                </span>{' '}
                {workExperience.months === 1 ? 'month' : 'months'} of professional experience
                <br />
                <span className="text-sm text-cyan-300">plus {internshipExperience.months} months of internship</span>
              </p>
            </div>
